...
def pick(paragraphs, select, k):
    ...
    def pick_helper(n, i):
        ...
        if (select(paragraphs[i])):
            ...
            return pick_helper(n - 1, i + 1)

        return pick_helper(n, i + 1)

    return pick_helper(k, 0)
    # END PROBLEM 1
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def about_check(word):
        splitWord = word.split(" ")
        for i in range(len(topic)):
            for j in range(len(splitWord)):
                currWord = (remove_punctuation(splitWord[j])).lower()
                if (topic[i].lower() == currWord):
                    return True
...
def wpm(typed, elapsed):
    ...
    return (wordCount / minutes_elapsed)
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    min_diff = diff_function(typed_word, curr_word, limit)
    min_word = curr_word

    for i in range(1, len(word_list)):
        curr_word = word_list[i]
        curr_diff = diff_function(typed_word, curr_word, limit)

        if (typed_word == curr_word):
            return typed_word
...
def feline_fixes(typed, source, limit):
    ...
    def feline_fixes_helper(limit, index):
        if (limit == -1 or index == max):
            return 0

        if (typed[index] != source[index]):
            return 1 + feline_fixes_helper(limit - 1, index + 1)
        
        return feline_fixes_helper(limit, index + 1)

    return difference + feline_fixes_helper(limit, 0)
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == goal:  # Fill in the condition
        ...
        substitute = is_different + minimum_mewtations(start[1:], goal[1:], limit - is_different)
        # BEGIN
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(len(typed)):
        if (typed[i] == prompt[i]):
            ...
            progress = total_correct / len(prompt)
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(1, len(times_per_player[i])):
            curr_player_times.append(times_per_player[i][j] - times_per_player[i][j - 1])
        
        times.append(curr_player_times)
...
def fastest_words(match):
    ...
    for j in range(len(get_all_times(match)[0])):
        ...
        for i in range(len(get_all_times(match))):
            ...
            if (time(match, i, j) < time(match, min_i, min_j)):
                min_i = i
                min_j = j

        players[min_i].append(get_word(match, min_j))
